# GarzaHive — Agent instructions

Coding-agent guide for this repository (Copilot, Claude Code, Cursor, and similar tools).

This file is the **tool-agnostic** source of agent conventions. Keep it in sync with `CLAUDE.md` when either changes.
...
| `CONTRIBUTING.md` | Human contributor setup, providers, channels, plugins |
...
Plugins must import only from `@garzahive/sdk` — never from `@/server/*`.
...
- User-facing strings need keys in locale files (at minimum keep parity; `en.json` is base)
...
- Drizzle migrations are immutable once shipped: `bun run db:generate` + `bun run db:migrate`. No `db:push`.
...
Use `createLogger('scope:name')` — prefer focused scopes over ad-hoc `console.log`.
...
- **Provider-agnostic core**: code outside `src/server/llm/{provider}/` and `plugins/` must not branch on provider names. Capabilities are declared by the provider itself.
...
Scaffold with `bunx create-garzahive-plugin`. Reference implementations: `plugins/teamspeak/`, `plugins/twilio-sms/`, `plugins/home-automation/`, `plugins/replicate/`. Import only from `@garzahive/sdk`.

## Git conventions

- **Never** include `Co-Authored-By` lines in commit messages
- Commit messages follow conventional commits: `feat:`, `fix:`, `refactor:`, `test:`, `docs:`, `chore:`, `site:`, `ci:`
...
5. Prefer `bun run test` (the npm script with an explicit sorted file list). Bare `bun test` discovery can leak global mock state between files.
6. Typecheck needs heap headroom: `NODE_OPTIONS=--max-old-space-size=8192` (already set in the `typecheck` script).
7. Build is **not** in the pre-commit hook (memory); CI runs `bun run build`.

## Local verification / safety

Read `testing-instance.md` before booting the app in this environment.

- Prefer verifying with the seeded test DB over typecheck-only.
- **Never** boot against the canonical seed dir directly — copy it first.
- This shell may inherit **PROD** env (`DB_PATH`, `GARZAHIVE_DATA_DIR`, `PORT=3000`). Always override `DB_PATH` + `GARZAHIVE_DATA_DIR` + `PORT` when running locally. Never kill processes by port/pattern on shared machines — kill your isolated PID only.
- Playwright: use `waitUntil: 'domcontentloaded'` (not `networkidle` — SSE keeps the network busy); chromium may need `args: ['--no-sandbox']`.
- Settings is a modal, not a route (open via URL hooks such as `/?email_connected=x` when automating).
...
bun run dev          # Start dev servers (Vite + Hono) — client :5173, API via proxy
...
bun run test         # Unit tests (bun test with sorted file list)
bun run db:generate  # Generate a Drizzle migration from schema changes
bun run db:snapshot  # Snapshot the DB (db:snapshot:list / db:snapshot:restore)
bun scripts/check-locales.ts   # Locale key parity
```
...
src/
  server/           # Backend (Bun + Hono)
    ...
    channels/       # Built-in ChannelAdapters (telegram, discord, slack, …)
...
site/               # Marketing site
data/               # Persistent data (SQLite DB, uploads, workspaces) — gitignored
```
...
| Do | Don't |
|---|---|
| Read the relevant `*.md` spec before changing that area | Invent parallel UI patterns when a shared component exists |
| Emit SSE when mutating visible state; handle every event you emit | Cast `as SSEEventType` to skip declaring events |
| Put shared types/constants in `src/shared/` | Hardcode UI strings or colors |
| Keep plugins on `@garzahive/sdk` only | Import `@/server/*` from plugins |
| Ship `docs-site/` + `api.md` with user-facing API/UI changes | Leave docs for "later" |
| Use conventional commits without `Co-Authored-By` | Hardcode model id lists for LLM providers |
| Override data dir/port when testing locally | Point bare `bun run start` / migrate at inherited PROD env |
