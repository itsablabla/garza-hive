...
describe('readAttachmentBlob', () => {
  it('throws for non-existent local file', async () => {
    const att: OutboundAttachment = {
      source: '/tmp/definitely-does-not-exist-garzahive-test-file.png',
      mimeType: 'image/png',
    }
  ...
  it('reads existing local file as blob', async () => {
    ...
    const tmpPath = `/tmp/garzahive-test-adapter-blob-${Date.now()}-${Math.random().toString(36).slice(2)}.txt`
