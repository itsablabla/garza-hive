 ...
 * `@garzahive/sdk`, never from `@/server/...` or `@/shared/...`)
 * and exercises every extension point through GarzaHive's host wiring:
 *
 ...
 * `@garzahive/sdk` will load + behave the same way under
 * GarzaHive's plugin manager.
 */
...
describe('SDK contract — external plugin end-to-end', () => {
  it('imports cleanly with only @garzahive/sdk in its module graph', () => {
  ...
  it('channel adapter integrates with the host channelAdapters registry', async () => {
    ...
    const sent = await adapter.sendMessage(
      'channel-1',
      {},
      { chatId: 'chat-9', content: 'hi from garzahive' },
    )
    expect(sent.platformMessageId).toMatch(/^ext-/)
    expect((sent.deliveryMeta as { content: string }).content).toBe('hi from garzahive')
  })
  ...
  it('fixture manifest is conformant with the JSON Schema', async () => {
    ...
    expect(manifest.garzahive).toBe('>=0.40.0')
