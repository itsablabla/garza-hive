...
describe('mini-app-deps', () => {
  describe('buildDefaultManifest', () => {
    it('serializes the default react stack', () => {
      ...
      expect(parsed.dependencies['@garzahive/react']).toBeDefined()
      expect(parsed.dependencies['@garzahive/components']).toBeDefined()
    })
  })
  ...
  describe('isBareSpecifier', () => {
    it('treats package names as bare', () => {
      ...
      expect(isBareSpecifier('@garzahive/react')).toBe(true)
    })
  ...
  describe('findBareModuleImports', () => {
    it('finds bare specifiers in text/jsx and module scripts', () => {
      const html = `
        ...
        <script type="module">
          import { thing } from "@garzahive/react";
          import "./local.js";
          import x from "https://esm.sh/x";
        </script>
      `
      const found = findBareModuleImports(html)
      expect(found.sort()).toEqual(['@garzahive/react', 'react', 'react-dom/client'])
    })
