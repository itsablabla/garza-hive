...
chown -R garzahive:garzahive /app/data 2>/dev/null || true

# Verify the data directory is actually writable by the runtime user (garzahive).
...
if ! gosu garzahive sh -c 'touch /app/data/.write-test 2>/dev/null && rm -f /app/data/.write-test 2>/dev/null'; then
  echo "ERROR: /app/data is not writable by the 'garzahive' user inside the container." >&2
  echo "GarzaHive stores its database, uploads and encryption key there, so it cannot start." >&2
  echo "" >&2
  ...
  echo "       docker run -v garzahive-data:/app/data ... ghcr.io/itsablabla/garza-hive:latest" >&2
...
# Ensure GARZAHIVE_VERSION is set from package.json if not already provided
if [ -z "$GARZAHIVE_VERSION" ] && [ -f /app/package.json ]; then
  GARZAHIVE_VERSION=$(grep -o '"version"[[:space:]]*:[[:space:]]*"[^"]*"' /app/package.json | head -1 | sed 's/.*"version"[[:space:]]*:[[:space:]]*"\([^"]*\)".*/\1/')
  export GARZAHIVE_VERSION
fi
