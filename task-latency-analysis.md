# Task latency — GarzaHive vs Claude Code (investigation 2026-05-29)

Why the *same* task is fast on Claude Code (CC) and slow on GarzaHive, even though
both use **Opus 4.8 on the same OAuth endpoint** (GarzaHive's anthropic-oauth
...
1. **Different thinking API generation.** CC uses the new adaptive/effort API
   ...
   trivial read). GarzaHive uses the **legacy** API (`thinking:{type:"enabled",
...
2. **Different architecture.** CC delegates the grunt exploration to a **Haiku
   sub-agent** (43 of 52 requests); its Opus only orchestrates (6 steps). GarzaHive
...
garzahive repo. Read-only → no repo state to reset → starting state identical.

## Runs

| run | model / thinking | round-trips | tool calls | calls/step | max batch | wall |
|---|---|---|---|---|---|---|
| `garzahive-normal` | Opus, budget 8192, interleaved on | ~96 | 109 | ~1.14 | — | 7m15 |
| `garzahive-low` | Opus, budget 2048, interleaved on | 98 | 115 | 1.28 | 4 | 7m31 |
| `garzahive-nointerleave` | Opus, budget 2048, interleaved **off** | 69 | 106 | 1.58 | 4 | 7m09 |
| **`claude-code`** | Opus orchestrator + **Haiku sub-agent**, effort=high adaptive | 52 (9 Opus + 43 Haiku) | ~54 | Opus **3.0** / Haiku 1.0 | 4 | **4m53** |
| **`garzahive-adaptive`** (after fix) | Opus, **adaptive** thinking, effort=medium | **24** | **41** | **1.78** | 4 | **2m44** |
...
GarzaHive-adaptive (effort=medium) already matches/beats CC on this task (CC at
...
GarzaHive sends `thinking: {type:"enabled", budget_tokens:8192}` (from
...
thinking tokens before every tool call." It's adaptive; GarzaHive's is fixed and
...
synthesized (6 thinking steps, batching 3.0 calls/step). GarzaHive ran all ~96
...
CC's Opus: 3.0 calls/step (every tool-step batched 2–4). GarzaHive's Opus: 1.3–1.6,
...
- **Tool count / schema size** — CC carries **more** (69–79 tools, 90–122 KB
  incl. MCP) than GarzaHive (33 tools, 24 KB) and is faster.
- **Interleaved thinking alone** — CC sends `interleaved-thinking-2025-05-14`
  too and still batches 3.0. The -30% we saw on GarzaHive when stripping it is the
...
### Betas CC sends that GarzaHive doesn't
...
request params. GarzaHive sends only 6 betas (`anthropic-oauth-auth.ts:264`). The
...
1. **Migrate the anthropic provider to the effort/adaptive thinking API.**
   **✅ DONE (2026-05-29).** `buildThinkingParams` in `_anthropic-shared.ts` now
   ...
   `config.llm.adaptiveThinking` is on (default; `GARZAHIVE_ADAPTIVE_THINKING=false`
...
CC has it easy: one provider, Haiku is always the cheap tier. GarzaHive is
...
- If nothing is configured, fall back to the main model (current behaviour) but
  ...
  cheapest model only works if GarzaHive tracks cost/speed tiers per model — it
...
```bash
# capture (no code change to GarzaHive — env var only)
bun scripts/llm-capture-proxy.ts --port 8789 --label <run>
ANTHROPIC_BASE_URL=http://localhost:8789 bun run dev     # GarzaHive
