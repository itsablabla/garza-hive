 ...
 *
 * `GARZAHIVE_GH_TOKEN` rides on `ctx.workspaceOverride.env` so the shell
 ...
 * Callers usually pass `{ ...process.env, GARZAHIVE_WORKSPACE: ..., ... }`
 * and rely on this helper to splice in `GARZAHIVE_GH_TOKEN` when the
