...
description: Rich interactive UIs generated by Agents, embedded directly in the GarzaHive sidebar.
---

Mini-apps are small web applications that Agents create and manage. They appear in the GarzaHive sidebar and can be opened as side panels or full pages, giving Agents the ability to build interactive UIs on the fly.

## What Are Mini-Apps?

A mini-app is a self-contained web application (HTML + JSX + CSS) stored in GarzaHive's database and served through its built-in web server. Agents create them using AI tools, no manual coding required.
...
- **Platform API**: build UI extensions that manage GarzaHive's own resources (contacts, crons, projects…) through the same REST API the settings pages use, gated per resource
...
```
┌─────────────────────────────────┐
│  GarzaHive UI (sidebar / panel)    │
│  ┌───────────────────────────┐  │
│  │  Mini-App (iframe)        │  │
│  │  • React + JSX            │  │
│  │  • @garzahive/react hooks    │  │
│  │  • @garzahive/components     │  │
│  └───────┬───────────────────┘  │
│          │ postMessage SDK      │
├──────────┼──────────────────────┤
│  GarzaHive Server                  │
...
Mini-apps run in an iframe and communicate with GarzaHive through a postMessage-based SDK. The SDK provides hooks for React, a component library, storage, theming, and more.
...
import { Card, Button, Stack } from "@garzahive/components";
...
- [Hooks](/docs/mini-apps/hooks/): React hooks for GarzaHive integration
