 ...
 * into the child process via `GARZAHIVE_GH_TOKEN`, and read by an inline
 ...
 * `$GARZAHIVE_GH_TOKEN` expands to the PAT. The helper ignores stdin and
...
export const CREDENTIAL_HELPER =
  '!f() { echo username=x-access-token; echo "password=$GARZAHIVE_GH_TOKEN"; }; f'
...
async function runClone(
  ...
  try {
    ...
    const proc = Bun.spawn(
      ...
      {
        env: { ...process.env, GARZAHIVE_GH_TOKEN: pat },
        stdout: 'pipe',
        stderr: 'pipe',
      },
    )
