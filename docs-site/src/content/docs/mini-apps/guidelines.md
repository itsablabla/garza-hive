...
- **Use `@garzahive/components`** which auto-adapt to the current theme
- **Never hardcode** `#ffffff` or `#000000` for backgrounds/text
- **Test both modes** by toggling the theme in GarzaHive settings
...
- **Support full-page mode**: use `isFullPage` from `useGarzaHive()` to adjust layout
- **Avoid horizontal scrolling**: keep content within the panel width

```jsx
function App() {
  const { isFullPage } = useGarzaHive();
...
Before building custom UI, check if `@garzahive/components` has what you need. The library includes 50+ components covering most common patterns:
...
- **Always call `useGarzaHive()` first** and wait for `ready`
