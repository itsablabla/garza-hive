...
| Périmètre d'expansion | **Opt-in par flag `expandsSecrets`** (décidé à l'implémentation P1) : seuls les tools dont les args sortent de la plateforme expansent (`run_shell`, `http_request`, `browse_url`, `screenshot_url`, `write_file`, `edit_file`, `multi_edit`) + custom/MCP tools toujours. **MCP server launch** (`command`/`args`/`env` in `mcp_servers`) also expands at connect time via `expandMcpLaunchSecrets` — this is not a tool call, so tool-executor alone is not enough. Partout ailleurs le placeholder reste du texte inerte — l'expansion uniforme aurait créé une auto-fuite : `memorize('{{secret:X}}')` aurait injecté la vraie valeur dans une mémoire ré-injectée en prompt |
