...
description: "How several people share one GarzaHive instance: accounts, invitations, the shared Agent session, and what each Agent knows about who is speaking."
---

GarzaHive is built for a household or a small group, not for one isolated user. Several people log in to the same instance, talk to the same Agents, and the Agents remember who is who. This page covers how accounts work, how to invite people, and what is shared between everyone versus what stays tied to an individual.
...
- A new account needs both a Better Auth login (email and password, minimum 8 characters) and a GarzaHive **profile** (first name, last name, pseudonym, interface language, and optionally the Agent language). A login without a profile cannot reach any protected route; the middleware blocks it until onboarding is complete.
...
2. GarzaHive produces an invitation token (a link).
...
This is the payoff of multi-user. When an Agent processes a turn, GarzaHive injects a **Current speaker** block into its system prompt for that turn: the speaker's name and pseudonym, plus any notes the Agent or the household has recorded about that person. If the Agent has never met this person, it is told to introduce itself and ask a couple of natural questions, then save what it learns so every Agent benefits.
