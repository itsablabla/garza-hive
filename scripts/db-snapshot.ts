 ...
 *
 * Snapshots live under `data/snapshots/<timestamp>[__label]/garzahive.db`.
 ...
 * the server at a fresh one (`rm data/garzahive.db && bun run db:migrate`),
...
const dbPath = resolve(process.env.DB_PATH ?? './data/garzahive.db')
...
function create(label?: string) {
  ...
  const targetPath = join(folderPath, 'garzahive.db')
...
function list() {
  ...
  const entries = readdirSync(snapshotsDir)
    .filter((name) => {
      const p = join(snapshotsDir, name, 'garzahive.db')
...
function restore(name: string) {
  ...
  const sourcePath = join(snapshotsDir, name, 'garzahive.db')
