...
4. In GarzaHive, add a Matrix channel with the access token and homeserver URL
...
- **Inbound:** GarzaHive uses Matrix's `/sync` endpoint with long-polling to receive events in real time. It filters for `m.room.message` events, extracts text and media, and routes to the Agent.
...
- **No public URL required**: Matrix uses client-side long-polling, so GarzaHive doesn't need to be publicly reachable
